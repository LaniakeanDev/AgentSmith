    ...
    def spawn(self, code: str) -> ExecutionResult:
        ...
        try:
                ...
                if output_data["success"] and "final_answer" in output_data:
                    ...
                    return ExecutionResult(
                        ...
                        output=output_data["output"] or ("No output from "
                                                         "execution"),
                        error=f"[spawner:100] Error during sandbox execution:"
                              f" {error}"
                    )
            except json.JSONDecodeError:
                error = f"[spawner:107] The sandbox output isn't "\
            ...
            return ExecutionResult(
                ...
                error=f"[spawner:112] Sandbox execution timed out after "
            ...
            return ExecutionResult(
                success=False,
                output="No output from execution",
                error=f"[spawner:119] {type(e).__name__}: {str(e)}",
            )

...
class SWEBenchTaskInput(BaseModel):
    ...
    eval_script: str = Field(..., description="Bash script to run inside the \
        container to evaluate the patch")
    hints_text: str = Field(default="", description="Optional hints about the \
        issue (may be empty)")
...
class StepMetrics(BaseModel):
    ...
    input_tokens: int = Field(..., description="Tokens sent to the LLM for \
        this step")
    ...
    model_name: str = Field(default="", description="Model identifier used \
        for this step (e.g., 'qwen/qwen3-235b-a22b-2507')")
    llm_output: str = Field(default="", description="Raw text generated by \
        the LLM before code extraction")
    sandbox_input: str = Field(default="", description="Python code sent to \
...
class SolutionOutput(BaseModel):
    ...
    solution.json.
    The moulinette validates this against task correctness and metrics
    limits.
    """
    ...
    success: bool = Field(..., description="Whether the agent believes it \
        solved the task")
    solution: str = Field(..., description="For MBPP: the Python function \
        code. For SWE-bench: the git patch (diff)")
